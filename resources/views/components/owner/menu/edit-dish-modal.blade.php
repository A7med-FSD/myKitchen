<div x-show="showEditDishModal" 
    ...
    <div x-show="showEditDishModal"
        ...
        <div class="bg-linear-to-r from-yellow-400 to-orange-400 p-6 relative">
            <h3 class="text-2xl font-bold text-gray-900">Edit Dish</h3>
            <p class="text-gray-900/80 text-sm mt-1">Update your menu item details</p>
            <button @click="closeEditDishModal()" class="absolute top-4 right-4 text-gray-900/60 hover:text-gray-900 hover:bg-white/20 rounded-full p-2 transition-colors cursor-pointer">
        ...
        <div class="p-6 space-y-5">
            ...
            <div>
                ...
                <div x-data="{ isDragging: false }"
                    ...
                    @drop.prevent="isDragging = false; handleEditImageUpload($event)"
                    ...
                    {{-- Preview --}}
                    <div x-show="editDish.imagePreview" class="relative">
                        <img :src="editDish.imagePreview" alt="Preview" class="w-full h-64 rounded-4xl p-5 object-cover">
                    ...
                    {{-- Upload Prompt --}}
                    <div x-show="!editDish.imagePreview" 
            ...
            {{-- Dish Name --}}
            <div>
                <label for="editDishName" class="block text-sm font-medium text-gray-700 mb-1">
                    Dish Name <span class="text-red-500">*</span>
                </label>
                <input type="text" 
                       id="editDishName" 
                       x-model="editDish.name" 
            ...
            {{-- Price --}}
            <div>
                <label for="editDishPrice" class="block text-sm font-medium text-gray-700 mb-1">
            ...
            {{-- Description --}}
            <div>
                <label for="editDishDescription" class="block text-sm font-medium text-gray-700 mb-1">
                    Description <span class="text-red-500">*</span>
                </label>
                <textarea id="editDishDescription" 
                          x-model="editDish.description" 
                ...
                <div class="flex items-center justify-between mt-1">
                    ...
                    <span class="text-xs text-gray-400" x-text="(editDish.description?.length || 0) + '/200'"></span>
                </div>
            </div>
            ...
            <div x-data="{ categoryOpen: false }">
                ...
                <div class="relative">
                    <button @click="categoryOpen = !categoryOpen" 
                            type="button"
                            :class="dishErrors.category ? 'border-red-400' : editDish.category ? 'border-yellow-400' : 'border-gray-200'"
                        ...
                        <span :class="editDish.category ? 'text-gray-900' : 'text-gray-400'" x-text="editDish.category || 'Select Category'"></span>
                    ...
                    <div x-show="categoryOpen" 
                        ...
                        <template x-for="cat in categories.filter(c => c.name !== 'All')" :key="cat.name">
                            <button @click="editDish.category = cat.name; categoryOpen = false; dishErrors.category = ''"
                                    type="button"
                                    class="w-full px-4 py-2 text-left hover:bg-yellow-50 transition-colors flex items-center gap-2 cursor-pointer"
                                    :class="editDish.category === cat.name ? 'bg-yellow-50 text-yellow-700' : 'text-gray-700'">
            ...
            <div>
                ...
                <div class="flex gap-2 mb-2">
                    <button @click="editDish.prepTime = 15" 
                            type="button"
                            :class="editDish.prepTime === 15 ? 'bg-yellow-400 text-gray-900 border-yellow-400' : 'bg-white text-gray-600 border-gray-200 hover:border-yellow-300'"
                            class="flex-1 px-3 py-2 rounded-lg border-2 font-medium transition-all cursor-pointer">15 min</button>
                    <button @click="editDish.prepTime = 30" 
                            type="button"
                            :class="editDish.prepTime === 30 ? 'bg-yellow-400 text-gray-900 border-yellow-400' : 'bg-white text-gray-600 border-gray-200 hover:border-yellow-300'"
                            class="flex-1 px-3 py-2 rounded-lg border-2 font-medium transition-all cursor-pointer">30 min</button>
                    <button @click="editDish.prepTime = 45" 
                            type="button"
                            :class="editDish.prepTime === 45 ? 'bg-yellow-400 text-gray-900 border-yellow-400' : 'bg-white text-gray-600 border-gray-200 hover:border-yellow-300'"
                            class="flex-1 px-3 py-2 rounded-lg border-2 font-medium transition-all cursor-pointer">45 min</button>
                    <button @click="editDish.prepTime = 60" 
                            type="button"
                            :class="editDish.prepTime === 60 ? 'bg-yellow-400 text-gray-900 border-yellow-400' : 'bg-white text-gray-600 border-gray-200 hover:border-yellow-300'"
            ...
            <div x-data="{ badgeOpen: false }">
                ...
                <div class="relative">
                    <button @click="badgeOpen = !badgeOpen" 
                        ...
                        <span :class="editDish.badge ? 'text-gray-900' : 'text-gray-400'" x-text="editDish.badge || 'None'"></span>
                    ...
                    <div x-show="badgeOpen" 
                        ...
                        <button @click="editDish.badge = null; badgeOpen = false"
                                type="button"
                                class="w-full px-4 py-2 text-left hover:bg-gray-50 transition-colors cursor-pointer"
                                :class="!editDish.badge ? 'bg-gray-50 text-gray-700' : 'text-gray-600'">
                            None
                        </button>
                        <button @click="editDish.badge = 'new'; badgeOpen = false"
                                type="button"
                                class="w-full px-4 py-2 text-left hover:bg-blue-50 transition-colors cursor-pointer"
                                :class="editDish.badge === 'new' ? 'bg-blue-50 text-blue-700' : 'text-gray-700'">
                            🆕 New
                        </button>
                        <button @click="editDish.badge = 'featured'; badgeOpen = false"
                                type="button"
                                class="w-full px-4 py-2 text-left hover:bg-purple-50 transition-colors cursor-pointer"
                                :class="editDish.badge === 'featured' ? 'bg-purple-50 text-purple-700' : 'text-gray-700'">
                            ⭐ Featured
                        </button>
                        <button @click="editDish.badge = 'recommended'; badgeOpen = false"
                                type="button"
                                class="w-full px-4 py-2 text-left hover:bg-green-50 transition-colors cursor-pointer"
                                :class="editDish.badge === 'recommended' ? 'bg-green-50 text-green-700' : 'text-gray-700'">
                            👍 Recommended
                        </button>
                        <button @click="editDish.badge = 'special'; badgeOpen = false"
                                type="button"
                                class="w-full px-4 py-2 text-left hover:bg-orange-50 transition-colors cursor-pointer"
                                :class="editDish.badge === 'special' ? 'bg-orange-50 text-orange-700' : 'text-gray-700'">
                            ✨ Special
                        </button>
                    </div>
                </div>
        ...
        <div class="p-6 bg-gray-50 border-t border-gray-200 flex gap-3">
            ...
            <button @click="closeEditDishModal()" 
                    type="button"
