<div x-show="showAddPromoModal" 
    ...
    <div x-show="showAddPromoModal"
        ...
        <div class="modal-header">
            <h3 class="modal-header-title">Add New Promotion</h3>
            <p class="modal-header-subtitle">Create a special offer for your customers</p>
            <button @click="closeAddPromoModal()" class="modal-close">
        ...
        <div class="modal-body-content">
            ...
            {{-- Promo Title --}}
            <div>
                <label for="promoTitle" class="block text-sm font-medium text-gray-700 mb-1">
                    Promotion Title <span class="text-red-500">*</span>
                </label>
                <input type="text" 
                       id="promoTitle" 
                       x-model="newPromo.title" 
            ...
            {{-- Promo Code --}}
            <div>
                <label for="promoCode" class="block text-sm font-medium text-gray-700 mb-1">
                    Promo Code <span class="text-red-500">*</span>
                </label>
                <input type="text" 
                       id="promoCode" 
                       x-model="newPromo.code" 
                       @input="newPromo.code = newPromo.code.toUpperCase(); promoErrors.code = ''"
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div x-data="{ openTypeDropdown: false }" class="relative">
                    ...
                    <button type="button" 
                        ...
                        <span x-text="newPromo.discountType === 'percentage' ? 'Percentage (%)' : 'Fixed Amount ($)'"></span>
                    ...
                    <div x-show="openTypeDropdown" 
                        ...
                        <button type="button" 
                                @click="newPromo.discountType = 'percentage'; openTypeDropdown = false"
                                class="dropdown-item"
                                :class="newPromo.discountType === 'percentage' ? 'dropdown-item-active' : ''">
                            Percentage (%)
                        </button>
                        <button type="button" 
                                @click="newPromo.discountType = 'fixed'; openTypeDropdown = false"
                                class="dropdown-item"
                                :class="newPromo.discountType === 'fixed' ? 'dropdown-item-active' : ''">
                            Fixed Amount ($)
                        </button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" 
                               id="discountValue" 
                               x-model="newPromo.discountValue" 
                               @input="promoErrors.discountValue = ''"
                               placeholder="0"
                               min="0"
                               :max="newPromo.discountType === 'percentage' ? 100 : 9999"
                               step="1"
                               ...
                               class="w-full px-4 py-2 rounded-xl border focus:ring-2 outline-none transition-all placeholder:text-gray-300">
                        <span class="absolute right-8 top-1/2 -translate-y-1/2 text-gray-500 text-sm font-medium" x-text="newPromo.discountType === 'percentage' ? '%' : '$'"></span>
                    </div>
            ...
            <div>
                ...
                <div x-data="{ openApplyToDropdown: false }" class="relative">
                    <button type="button" 
                        ...
                        <span class="capitalize" x-text="newPromo.applyTo === 'all' ? 'All Menu Items' : newPromo.applyTo === 'category' ? 'Specific Category' : 'Specific Dish'"></span>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5 text-gray-400">
                            <path fill-rule="evenodd" d="M5.22 8.22a.75.75 0 0 1 1.06 0L10 11.94l3.72-3.72a.75.75 0 1 1 1.06 1.06l-4.25 4.25a.75.75 0 0 1-1.06 0L5.22 9.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" />
                        </svg>
                    </button>
                    ...
                    <div x-show="openApplyToDropdown" 
                         ...
                         class="dropdown-menu top-full mt-2 w-full">
                        <button type="button" 
                                @click="newPromo.applyTo = 'all'; newPromo.selectedCategory = null; newPromo.selectedDish = null; openApplyToDropdown = false; promoErrors.selectedCategory = ''; promoErrors.selectedDish = ''"
                                class="dropdown-item"
                                :class="newPromo.applyTo === 'all' ? 'dropdown-item-active' : ''">
                            All Menu Items
                        </button>
                        <button type="button" 
                                @click="newPromo.applyTo = 'category'; newPromo.selectedDish = null; openApplyToDropdown = false; promoErrors.selectedDish = ''"
                                class="dropdown-item"
                                :class="newPromo.applyTo === 'category' ? 'dropdown-item-active' : ''">
                            Specific Category
                        </button>
                        <button type="button" 
                                @click="newPromo.applyTo = 'dish'; newPromo.selectedCategory = null; openApplyToDropdown = false; promoErrors.selectedCategory = ''"
                                class="dropdown-item"
                                :class="newPromo.applyTo === 'dish' ? 'dropdown-item-active' : ''">
                            Specific Dish
                        </button>
                    </div>
                </div>
            ...
            <div x-show="newPromo.applyTo === 'category'" x-transition>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                    Select Categories <span class="text-red-500">*</span>
                </label>
                
                <div class="space-y-3">
                    <template x-for="(selectedCat, index) in newPromo.selectedCategories" :key="index">
                        <div class="flex gap-2 items-start">
                            <div x-data="{ openCategoryDropdown: false }" class="relative flex-1">
                                <div class="relative">
                                    <input type="text" 
                                           :value="newPromo.selectedCategories[index] || newPromo.categorySearches[index]"
                                           @input="newPromo.categorySearches[index] = $event.target.value; newPromo.selectedCategories[index] = ''; openCategoryDropdown = true; if (promoErrors.selectedCategories) promoErrors.selectedCategories[index] = ''"
                                ...
                                <div x-show="openCategoryDropdown && getFilteredCategories(index).length > 0" 
                                    ...
                                    <template x-for="category in getFilteredCategories(index)" :key="category.name">
                                        <button type="button" 
                                                @click="newPromo.selectedCategories[index] = category.name; newPromo.categorySearches[index] = ''; openCategoryDropdown = false; if (promoErrors.selectedCategories) promoErrors.selectedCategories[index] = ''"
                                                class="dropdown-item"
                                                :class="newPromo.selectedCategories[index] === category.name ? 'dropdown-item-active' : ''">
                                            <span x-text="category.icon + ' ' + category.name"></span>
                                        </button>
                                    </template>
                                </div>
                                
                                <div x-show="openCategoryDropdown && getFilteredCategories(index).length === 0 && newPromo.categorySearches[index]" 
                                     x-transition
                                    ...
                                    No category found matching "<span x-text="newPromo.categorySearches[index]"></span>"
                                </div>
                            ...
                            <button x-show="index > 0" 
                                    type="button" 
                                    @click="removeCategoryField(index, 'new')"
            ...
            <div x-show="newPromo.applyTo === 'dish'" x-transition>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                    Select Dishes <span class="text-red-500">*</span>
                </label>
                
                <div class="space-y-3">
                    <template x-for="(selectedDsh, index) in newPromo.selectedDishes" :key="index">
                        <div class="flex gap-2 items-start">
                            <div x-data="{ openDishDropdown: false }" class="relative flex-1">
                                <div class="relative">
                                    <input type="text" 
                                           :value="newPromo.selectedDishes[index] || newPromo.dishSearches[index]"
                                           @input="newPromo.dishSearches[index] = $event.target.value; newPromo.selectedDishes[index] = ''; openDishDropdown = true; if (promoErrors.selectedDishes) promoErrors.selectedDishes[index] = ''"
                                ...
                                <div x-show="openDishDropdown && getFilteredDishes(index).length > 0" 
                                    ...
                                    <template x-for="dish in getFilteredDishes(index)" :key="dish.name">
                                        <button type="button" 
                                                @click="newPromo.selectedDishes[index] = dish.name; newPromo.dishSearches[index] = ''; openDishDropdown = false; if (promoErrors.selectedDishes) promoErrors.selectedDishes[index] = ''"
                                                class="dropdown-item text-left flex flex-col items-start gap-0.5"
                                                :class="newPromo.selectedDishes[index] === dish.name ? 'dropdown-item-active' : ''">
                                ...
                                <div x-show="openDishDropdown && getFilteredDishes(index).length === 0 && newPromo.dishSearches[index]" 
                                     x-transition
                                     class="absolute z-50 top-full mt-2 w-full bg-white rounded-2xl shadow-xl border border-gray-100 p-4 text-center text-sm text-gray-500">
                                    No dish found matching "<span x-text="newPromo.dishSearches[index]"></span>"
                                </div>
                            ...
                            <button x-show="index > 0" 
                                    type="button" 
                                    @click="removeDishField(index, 'new')"
                                    class="cursor-pointer p-2 rounded-lg text-red-500 hover:bg-red-50 transition-colors mt-0.5">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                                    <path fill-rule="evenodd" d="M8.75 1A2.75 2.75 0 0 0 6 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 1 0 .23 1.482l.149-.022.841 10.518A2.75 2.75 0 0 0 7.596 19h4.807a2.75 2.75 0 0 0 2.742-2.53l.841-10.52.149.023a.75.75 0 0 0 .23-1.482A41.03 41.03 0 0 0 14 4.193V3.75A2.75 2.75 0 0 0 11.25 1h-2.5ZM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4ZM8.58 7.72a.75.75 0 0 0-1.5.06l.3 7.5a.75.75 0 1 0 1.5-.06l-.3-7.5Zm4.34.06a.75.75 0 1 0-1.5-.06l-.3 7.5a.75.75 0 1 0 1.5.06l.3-7.5Z" clip-rule="evenodd" />
                                </svg>
            ...
            <div class="grid grid-cols-2 gap-4">
                {{-- Start Date Picker --}}
                <div x-data="datePicker(newPromo.startDate)" x-modelable="selectedDate" x-model="newPromo.startDate">
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                ...
                {{-- End Date Picker --}}
                <div x-data="datePicker(newPromo.endDate)" x-modelable="selectedDate" x-model="newPromo.endDate">
                    <label class="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" 
                           x-model="newPromo.isActive"
                           class="w-5 h-5 rounded border-gray-300 text-yellow-400 focus:ring-yellow-200">
                    <span class="form-label mb-0">Activate promotion immediately</span>
                </label>
        ...
        <div class="modal-footer">
            <button @click="closeAddPromoModal()" 
                    type="button"
                    class="modal-btn-cancel">
                Cancel
            </button>
            <button @click="savePromo()" 
                    type="button"
